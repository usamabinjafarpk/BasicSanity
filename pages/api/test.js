...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        try {
            ...
            const profileIcon = await driver.findElement(By.id('userMenuItem'));
            ...
            await driver.findElement(By.id('logoutPopupYesbtn')).click();

            await driver.navigate().refresh();

        await driver.findElement(By.id('usernameBox')).sendKeys(username); 
            await driver.findElement(By.id('passwordBox')).sendKeys(password);
            
            await driver.findElement(By.id('btn-fam')).click();
            await driver.sleep(30000); // Increase wait time if needed
            const screenshot5 = await driver.takeScreenshot();
            const screenshotPath5 = path.join(process.cwd(), 'public', 'assessor5.png');
            fs.writeFileSync(screenshotPath5, screenshot5, 'base64');

            await driver.findElement(By.id('marking_btn')).click();
            await driver.sleep(30000); // Increase wait time if needed
            await driver.findElement(By.id('getNewResponseButton')).click();
            await driver.sleep(30000); // Increase wait time if needed
            const screenshot6 = await driver.takeScreenshot();
            const screenshotPath6 = path.join(process.cwd(), 'public', 'assessor6.png');
            fs.writeFileSync(screenshotPath6, screenshot6, 'base64');
            

            await driver.findElement(By.id('res_0_ResponseIdColumn_0')).click();
            await driver.sleep(30000); // Increase wait time if needed

            const screenshot7 = await driver.takeScreenshot();
            const screenshotPath7 = path.join(process.cwd(), 'public', 'assessor76.png');
